...
class BaseDataset(Dataset):
    ...
    def __init__(self, options, dataset, ignore_3d=False, use_augmentation=True, is_train=True):
        ...
        #self.keypoints = np.concatenate([keypoints_openpose, keypoints_gt], axis=1) # for testing we have 4 views
    ...
    def __getitem__(self, index):
        ...
        # Load image
        #print(self.img_dir)
        #print(str(self.imgname[index][0],encoding='utf-8'))
        #print(join(self.img_dir, str(self.imgname[index][0],encoding='utf-8')))
        view_number = 4
        for i in range(view_number): # four views
            #print(type(self.imgname[index][i]).__name__)
            ...
            #imgname = join(self.img_dir,  self.imgname[index][7:])
            #print(imgname)
            #print(index, imgname) 
            #imgname = imgnames[i]
            try:
        ...
        #    item['pose_3d'] = torch.from_numpy(self.j3d_processing(S[0],rot,flip)).float()
        #else:
        ...
        if self.has_pose_3d:
            ...
            for i in range(4):
                item['pose_3d_%d' % i]  = torch.zeros(24,4,dtype=torch.float32)
        # Get 2D pose
